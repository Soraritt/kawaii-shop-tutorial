...
func (f *userReq) Customer() (IInsertUser, error) {
	...
	defer cancel() //insert ภายใน 5 วิ  ถ้า  return calcel และคืนทรัพยากร
	...
	if err := f.db.QueryRowContext(
		...
		f.req.Password,
		f.req.Username,
	).Scan(&f.id); err != nil { // อะไรที่  return .scan check
		switch err.Error() {
